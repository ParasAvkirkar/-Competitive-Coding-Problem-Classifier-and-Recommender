...
try:
    with open('current_progress.pickle', 'r+b') as f:
        ...
        print 'Resuming Progress from problem ' + sections[current_progress['section']] + ' and problem no ' + str(
...
try:
            ...
            if row.has_attr('class') and 'problemrow' in row['class']:
                ...
                for a in allHrefTags:
                    if a.has_attr('href') and 'problem' in a['href']:
                        ...
                        if p:
                            sqlDB.insert_problem_db('codechef_problem', p.name, p.url, p.statement, p.tags,
                                                    p.difficulty, p.category,
                                                    p.submission_size, p.constraints, p.time_limit, p.source_limit,
                                                    p.example_given)
    ...
    exc_type, exc_obj, exc_tb = sys.exc_info()
    print 'Exception at line ' + str(exc_tb.tb_lineno)
    logging.error('Time: {0} File: {1} Line: {2} Caused By: {3} Problem href: {4}'.format(datetime.datetime.now(),
