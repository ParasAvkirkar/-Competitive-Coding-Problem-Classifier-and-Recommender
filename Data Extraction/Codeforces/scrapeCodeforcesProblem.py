...
try:
    with open('current_progress.pickle', 'r+b') as f:
        ...
        print 'Resuming Progress from page ' + str(current_progress['link']) + ' and problem no ' + str(
            current_progress['problem_no'])
except Exception as e:
    print(e)
    exc_type, exc_obj, exc_tb = sys.exc_info()
    print 'Exception at line ' + str(exc_tb.tb_lineno)
    logging.error(
        'Time: {0} File: {1} Line: {2} Caused By: {3}'.format(datetime.datetime.now(), os.path.basename(__file__),
                                                              exc_tb.tb_lineno, e))
...
try:
        ...
        for problem in problemList[page_count:]:
            try:
                ...
                if p:
                    ...
                    # 	pickle.dump(p, f)
                    sqlDB.insert_problem_db('codeforces_problem', p.name, p.url, p.problemStatement, p.tags,
                    ...
                    with open('current_progress.pickle', 'w+b') as f:
                        pickle.dump(
                            {'link': codeforcesProblemsLink, 'problem_no': page_count, 'total_problems': global_count},
                            f)
                ...
                if page_count != 0:
                    ...
                    print 'Exception at line ' + str(exc_tb.tb_lineno)
                    logging.error(
                        'Time: {0} File: {1} Line: {2} ProblemsListPageUrl: {3} ProblemUrl: {4} Caused By: {5}'.format(
                            datetime.datetime.now(), os.path.basename(__file__),
    ...
    print 'Exception at line ' + str(exc_tb.tb_lineno)
    logging.error(
        'Time: {0} File: {1} Line: {2} ProblemsListPageUrl: {3} Caused By: {4}'.format(datetime.datetime.now(),
                                                                                       os.path.basename(__file__),
                                                                                       exc_tb.tb_lineno,
                                                                                       codeforcesProblemsLink, e))
# logging.error(str(datetime.datetime.now()) + ' :File Name: '+ str(os.path.basename(__file__)) +
# 		' :Line Number: '+ str(exc_tb.tb_lineno) + ' :Caused By: ' + str(e))
